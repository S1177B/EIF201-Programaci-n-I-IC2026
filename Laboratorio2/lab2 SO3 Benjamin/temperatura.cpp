...
{
    std::cout << "Sistema de Registro de Temperaturas\n";
    std::cout << "Ingrese la cantidad de dias: ";
    std::cin >> cantidadDias;

    while (cantidadDias <= 0)
    {
        std::cout << "Error. Ingrese un numero mayor que 0: ";
        std::cin >> cantidadDias;
    }
...
void mostrarResultados(const double* registro, int cantidad)
{
    ...
    std::cout << "Temperatura maxima: " << calcularMaximo(registro, cantidad) << " C\n";

    std::cout << "Temperatura minima: " << calcularMinimo(registro, cantidad) << " C\n";
}